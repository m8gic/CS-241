...
void logic() {
    ...
    case 2:
 To paste in Vim, use
p (after cursor) or P (before cursor) for text from Vim's internal registers (yanked with y or deleted with d), or use your terminal's paste shortcut (like Ctrl+Shift+V) for external text, often after entering insert mode or using :set paste for proper indentation. For system clipboard access within Vim, use "+p or "*p, and use Ctrl+R followed by Ctrl+P (in insert mode) for auto-indentation.        x++;
        break;
    case 3:
        y--;
        break;
