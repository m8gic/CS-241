...
int main(int argc, char *argv[]) {
    ...
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "preorderprint") == 0) {
        ...
        } else if (strcmp(argv[i], "numleaves") == 0) {
            printf("%d\n", numLeaves(myTree->root));
        }
    }
...
// returns true if-and-only-if the subtree pointed to by
// the paramter curr is a "binary search tree", that is, it
// has the symmetric ordering covered in lecture
bool isBinarySearchTree(Node *curr) {
    return isBSTHelper(curr, INT_MIN, INT_MAX);  // INT_MIN, INT_MAX
}

// TODO
// return the number of nodes in the tree that are leaves
int numLeaves(Node *curr) {
    if (curr == NULL) {
        return 0;
    }
...
bool insertChildNode(Node *n, int parentData, char branch, int childData) {
    ...
    return insertChildNode(n->left, parentData, branch, childData) ||
...
void processStdIn(BinaryTree *bt) {
    ...
    while (numChildInsertions-- > 0) {
        ...
        if (currBranch == 'l' || currBranch == 'r')
            insertChildBinaryTree(bt, currParentNode, currBranch,
                                  currChildNode);
        else
            cleanExit(bt, 1);
    }
...
bool insertChildBinaryTree(BinaryTree *bt, int parentData, char branch,
                           int childData) {
